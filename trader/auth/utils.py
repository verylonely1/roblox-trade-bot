...
class privUtils:
    def _getMetaDataChallengeId(resp: Union[requests.Response, aiohttp.ClientResponse], VAR_DICT: dict):
        """Private method. """
        try:
            return json.loads(base64.b64decode(resp.headers.get('rblx-challenge-metadata')).decode("UTF-8")).get('challengeId')
        except:
            return ""

    async def _prepareMetaData(resp: Union[requests.Response, aiohttp.ClientResponse], VAR_DICT: dict):
        """Private method. """
        Verification_MetaData = {
                    'verificationToken': (await resp.json())['verificationToken'],
                    'rememberDevice': True,
    ...
    async def _rawMetaData(resp: Union[requests.Response, aiohttp.ClientResponse], VAR_DICT: dict):
        """Private method. """
        Verification_MetaData = {
                    'verificationToken': (await resp.json())['verificationToken'],
                    'rememberDevice': True,
...
class Validate:
    @staticmethod
    def validate_types(func: Callable):
        async def wrapper(*args, **kwargs):
            await Validate._types(*args, **kwargs, funcname=func.__name__)
    ...
    @staticmethod
    def validate_tag(func: Callable):
        async def wrapper(*args, **kwargs):
            await Validate._tag(*args, **kwargs, funcname=func.__name__)
            result = await func(*args, **kwargs)
            return result
        return wrapper

    @staticmethod
    async def _types(*args, **kwargs) -> bool:
        if args:
            args = list(args)
    ...
    @staticmethod
    async def _tag(*args, **kwargs) -> bool:
        if len(args) > 1:
